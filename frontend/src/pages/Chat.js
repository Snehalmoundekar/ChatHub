...
function Chat() {
  ...
  input.onchange = (e) => {
    ...
    // ---------------- IMAGES ----------------
    if (type === "image") {
      setSelectedVideos([]);      // clear other file types
      setPreviewVideos([]);
      setSelectedDoc(null);
      setPreviewDoc(null);
    ...
    // ---------------- VIDEOS ----------------
    if (type === "video") {
      setSelectedImages([]);      // clear other file types
      setPreviewImages([]);
      setSelectedDoc(null);
      setPreviewDoc(null);
  ...
  //     // ✅ 2. Emit socket event so sidebar & receiver update
  //     // socket.emit("sendMessage", {
  //     //   sender: currentUser._id,
  //     //   receiver: selectedUser._id,
  //     //   text: res.data.text || "",
  ...
  const sendMultipleImages = async () => {
    ...
    selectedImages.forEach((img) => {
      formData.append("images", img);    // 👈 name = images
    });
    ...
    try {
      ...
      // ✅ 3. Update sidebar immediately
      setUsers((prevUsers) => {
        let last = res.data[res.data.length - 1];   // 👈 last uploaded file OR text
  ...
  const sendMultipleVideos = async () => {
    if (selectedVideos.length === 0 && !text.trim()) return;

    if (selectedVideos.length > 5) {
      alert("You can upload a maximum of 5 video at a time.");
      return;
    }
    const formData = new FormData();
    formData.append("sender", currentUser._id);
    ...
    selectedVideos.forEach((video) => {
      formData.append("videos", video);    // 👈 name = videos
    });
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/api/chat/upload-multiple-videos",
        formData,
        { headers: { "Content-Type": "multipart/form-data" } }
      );
  ...
  const loadMessages = async (user) => {
    ...
    // ✅ FIXED key names for markAsSeen event
  ...
  const sendMessage = () => {
    ...
    // ✅ FIXED senderId → sender, receiverId → receiver
    socket.emit("sendMessage", {
      sender: currentUser._id,
      receiver: selectedUser._id,
      text,
  ...
  return (
    <div className={`chat-container ${showProfilePage ? "profile-open" : ""}`}>
      ...
      <ChatWindow
        ...
        sendMultipleVideos={sendMultipleVideos}
